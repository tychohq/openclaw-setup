...
# Helper: parse identity JSON from sts get-caller-identity
# Sets: AWS_ACCOUNT_ID, AWS_USER_ARN, AWS_USER
parse_identity() {
    local json="$1"
    AWS_ACCOUNT_ID=$(echo "$json" | grep -o '"Account": "[^"]*"' | cut -d'"' -f4)
    AWS_USER_ARN=$(echo "$json" | grep -o '"Arn": "[^"]*"' | cut -d'"' -f4)
    AWS_USER=$(echo "$AWS_USER_ARN" | rev | cut -d'/' -f1 | rev)
}

# Helper: write S3 backend.tf for Terraform
write_backend_tf() {
...
# Helper: list available AWS profiles from credentials and config files
list_aws_profiles() {
...
# Helper: detect MFA device and prompt for ARN + code
# Sets: MFA_SERIAL, MFA_CODE
prompt_mfa() {
...
# Helper: get session token using MFA and export credentials
# Requires: MFA_SERIAL, MFA_CODE (call prompt_mfa first)
...
CONFIG_JSON=""
ENV_CONTENT=""
# Save .env values before resetting (interactive mode needs blank defaults)
_ENV_ASSISTANT_NAME_SAVED="${ASSISTANT_NAME:-}"
_ENV_OWNER_NAME_SAVED="${OWNER_NAME:-}"
_ENV_TIMEZONE_SAVED="${TIMEZONE:-}"
ASSISTANT_NAME=""
OWNER_NAME=""
TIMEZONE="UTC"
TF_VAR_ARGS=""
...
if [ "$CONFIG_CHOICE" = "1" ]; then
    ...
    if [ "$AUTO_MODE" = true ]; then
        ...
        ASSISTANT_NAME="${_ENV_ASSISTANT_NAME_SAVED:-OpenClaw}"
        OWNER_NAME="${_ENV_OWNER_NAME_SAVED:-}"
        TIMEZONE="${_ENV_TIMEZONE_SAVED:-America/New_York}"
