...
if [[ " $* " == *" --destroy "* ]] || [ "${1:-}" = "--destroy" ]; then
    ...
    if [ "$TFSTATE_S3" = "true" ] && [ -n "${DEPLOYMENT_NAME:-}" ] && [ -n "${AWS_ACCOUNT_ID:-}" ]; then
        TFSTATE_BUCKET="${DEPLOYMENT_NAME}-tfstate-${AWS_ACCOUNT_ID}"
        TFSTATE_REGION="${AWS_REGION:-us-east-1}"
        TFSTATE_DYNAMODB="${DEPLOYMENT_NAME}-tfstate-lock"
...
if [ "$TFSTATE_S3" = "true" ]; then
    _SCAN_BUCKET="${DEPLOYMENT_NAME}-tfstate-${AWS_ACCOUNT_ID}"
...
if [ "${TFSTATE_S3:-true}" = "true" ]; then
    echo "  Terraform State:  S3 (${DEPLOYMENT_NAME}-tfstate-${AWS_ACCOUNT_ID})"
else
