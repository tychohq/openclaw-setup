...
# Helper: slugify a string for S3/DynamoDB naming
# Usage: slugify "Brenner Spear" → "brenner-spear"
slugify() {
    echo "$1" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/-/g' | sed 's/--*/-/g' | sed 's/^-//;s/-$//'
}

# Helper: build tfstate bucket name from owner + assistant identity
# Usage: tfstate_bucket_name → sets TFSTATE_BUCKET and TFSTATE_DYNAMODB
tfstate_bucket_name() {
    local owner_slug assistant_slug
    owner_slug=$(slugify "${OWNER_NAME:-}")
    assistant_slug=$(slugify "${ASSISTANT_NAME:-openclaw}")

    if [ -n "$owner_slug" ] && [ -n "$assistant_slug" ]; then
        TFSTATE_BUCKET="${owner_slug}-${assistant_slug}-tfstate"
    elif [ -n "$owner_slug" ]; then
        TFSTATE_BUCKET="${owner_slug}-tfstate"
    else
        TFSTATE_BUCKET="${DEPLOYMENT_NAME}-tfstate"
    fi
    TFSTATE_DYNAMODB="${TFSTATE_BUCKET}-lock"
}
...
if [[ " $* " == *" --destroy "* ]] || [ "${1:-}" = "--destroy" ]; then
    ...
    # Re-generate backend.tf if S3 backend is configured
    TFSTATE_S3="${TFSTATE_S3:-true}"

    # Get AWS account ID if needed for S3 backend
    if [ "$TFSTATE_S3" = "true" ] && [ -z "${AWS_ACCOUNT_ID:-}" ]; then
        set +e
        AWS_ACCOUNT_ID=$(aws sts get-caller-identity --query Account --output text 2>/dev/null)
        set -e
    fi

    if [ "$TFSTATE_S3" = "true" ] && [ -n "${DEPLOYMENT_NAME:-}" ] && [ -n "${AWS_ACCOUNT_ID:-}" ]; then
        TFSTATE_REGION="${AWS_REGION:-us-east-1}"
        TFSTATE_KEY="terraform.tfstate"

        # Reconstruct bucket name from owner + assistant identity
        tfstate_bucket_name
...
# Check Terraform state (local or S3)
TFSTATE_S3="${TFSTATE_S3:-true}"
...
if [ "$TFSTATE_S3" = "true" ]; then
    tfstate_bucket_name
    _SCAN_BUCKET="$TFSTATE_BUCKET"
    echo -n "  S3 state bucket ($_SCAN_BUCKET)... "
    set +e
    aws s3api head-bucket --bucket "$_SCAN_BUCKET" --region "$AWS_REGION" 2>/dev/null
    _SCAN_BUCKET_EXISTS=$?
    set -e
    if [ $_SCAN_BUCKET_EXISTS -eq 0 ]; then
        echo -e "${YELLOW}Found${NC}"
        EXISTING_OPENCLAW+=("S3 state bucket: $_SCAN_BUCKET")
    else
        echo -e "${GREEN}None${NC}"
    fi
fi
...
if [ "${TFSTATE_S3:-true}" = "true" ]; then
    tfstate_bucket_name
    echo "  Terraform State:  S3 ($TFSTATE_BUCKET)"
else
...
# Create S3 bucket + DynamoDB table for remote state, then generate backend.tf.
# Bucket name: {deployment_name}-tfstate-{account_id}
# State key:   terraform.tfstate
# Opt out with TFSTATE_S3=false in .env

TFSTATE_S3="${TFSTATE_S3:-true}"
TFSTATE_REGION="${AWS_REGION}"
TFSTATE_KEY="terraform.tfstate"

# Build bucket name from owner + assistant identity
# e.g. "Brenner Spear" + "Axel" → brenner-spear-axel-tfstate
tfstate_bucket_name

HAS_LOCAL_STATE=false
[ -f terraform.tfstate ] && [ -s terraform.tfstate ] && HAS_LOCAL_STATE=true
if [ -n "${DEPLOY_DIR:-}" ] && [ -f "$DEPLOY_DIR/terraform.tfstate" ] && [ -s "$DEPLOY_DIR/terraform.tfstate" ]; then
    HAS_LOCAL_STATE=true
fi

if [ "$TFSTATE_S3" = "true" ]; then
    echo -e "${BLUE}Setting up remote state backend (S3)...${NC}"
    echo ""

    # Create S3 bucket if it doesn't exist
    set +e
    aws s3api head-bucket --bucket "$TFSTATE_BUCKET" --region "$TFSTATE_REGION" 2>/dev/null
    BUCKET_EXISTS=$?
    set -e

    if [ $BUCKET_EXISTS -ne 0 ]; then
        echo -n "  Creating S3 bucket: $TFSTATE_BUCKET... "
        if [ "$TFSTATE_REGION" = "us-east-1" ]; then
            aws s3api create-bucket \
                --bucket "$TFSTATE_BUCKET" \
                --region "$TFSTATE_REGION" > /dev/null 2>&1
        else
            aws s3api create-bucket \
                --bucket "$TFSTATE_BUCKET" \
                --region "$TFSTATE_REGION" \
                --create-bucket-configuration LocationConstraint="$TFSTATE_REGION" > /dev/null 2>&1
        fi
        echo -e "${GREEN}✓${NC}"
        ...
        aws s3api put-bucket-encryption \
            ...
            '{"Rules":[{"ApplyServerSideEncryptionByDefault":{"SSEAlgorithm":"AES256"},"BucketKeyEnabled":true}]}' > /dev/null 2>&1
        echo -e "${GREEN}✓${NC}"
        ...
        echo -e "  S3 bucket: ${GREEN}$TFSTATE_BUCKET (exists)${NC}"
    fi

    # Create DynamoDB table for state locking if it doesn't exist
    set +e
    aws dynamodb describe-table --table-name "$TFSTATE_DYNAMODB" --region "$TFSTATE_REGION" > /dev/null 2>&1
    TABLE_EXISTS=$?
    set -e

    if [ $TABLE_EXISTS -ne 0 ]; then
        echo -n "  Creating DynamoDB lock table: $TFSTATE_DYNAMODB... "
        aws dynamodb create-table \
            --table-name "$TFSTATE_DYNAMODB" \
        ...
        aws dynamodb wait table-exists --table-name "$TFSTATE_DYNAMODB" --region "$TFSTATE_REGION" 2>/dev/null
        echo -e "${GREEN}✓${NC}"
    else
        echo -e "  DynamoDB table: ${GREEN}$TFSTATE_DYNAMODB (exists)${NC}"
    fi
...
BACKEND_EOF
    echo ""
    echo -e "  ${GREEN}✓ Remote state backend configured${NC}"

    if [ "$HAS_LOCAL_STATE" = "true" ]; then
        echo -e "  ${YELLOW}Local state detected — will migrate to S3 during init${NC}"
    fi
    echo ""
else
    # Local state mode — remove backend.tf if it exists (from a previous S3 run)
    rm -f backend.tf

    # Legacy: link state files from deploy dir
...
# Initialize Terraform (with migration support for local → S3)
echo "  Initializing Terraform..."
if [ "$TFSTATE_S3" = "true" ] && [ "$HAS_LOCAL_STATE" = "true" ]; then
    echo "  Migrating local state to S3..."
    echo "yes" | terraform init -input=false -migrate-state > /dev/null 2>&1
else
    terraform init -input=false > /dev/null 2>&1
fi
...
# Copy state to deploy dir if using local state with external config
if [ "$TFSTATE_S3" != "true" ] && [ -n "${DEPLOY_DIR:-}" ] && [ "$DEPLOY_DIR" != "$SCRIPT_DIR/terraform" ]; then
